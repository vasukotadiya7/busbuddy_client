...
public class SeatViewActivity2 extends AppCompatActivity {
    private ImageView[] seats=new ImageView[31];
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_seat_view2);
        Instantiate();
        GetStringFromIntent();
        MakeColorSeat();
    }
...
//        booked=getIntent().getIntegerArrayListExtra("Booked");
    ...
    private  void bookseat(){
        ...
        Intent in =new Intent(SeatViewActivity2.this,TicketBooking.class);
...
try {
    ...
    int v=selected.indexOf(seatno);
    if(v==-1){

        selected.add(seatno);
        seats[seatno].setImageResource(R.drawable.ic_twotone_rectangle_selected);
    }else{
        seats[seatno].setImageResource(R.drawable.ic_twotone_rectangle_empty);
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                booked.forEach((k,v)-> {
                    try {
                        ...
                        if (v != 0 && v != 41) {
                            alreadybkd.add(v);
                            seats[v].setImageResource(R.drawable.ic_twotone_rectangle_booked);
                        }
                    }catch (NullPointerException e){
                        e.printStackTrace();
                    }
